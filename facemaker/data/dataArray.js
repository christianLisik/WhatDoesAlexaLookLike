...
function getLatinSquareTask() {
	...
	//setDataCookie("latinSquare", latinLetter);
		...
		$(xml).find('site').each(function(){
			...
			if ( id == _id) {
				...
				if( button != null || typeof button != "undefined"){ 
					document.getElementById("buttonHolder").innerHTML = "<a id='okay' class='play-btn' href='#'>" +  $(this).find('text[lang="' + selectedLanguage + '"]').attr('button') + "</a>";
					$("#okay").click(function(e) {
						$("#infoBoxBackground").fadeOut(200, function(){
							...
							if(_id == "todo"){
								...
								var voiceSamples=document.getElementById("voiceSample");
								var createAudioNode=document.createElement("audio");
								var audioNode=voiceSample.appendChild(createAudioNode);
								
								audioNode.setAttribute("autoplay","");
								audioNode.setAttribute("controls","");
								
								var createAudioNode=document.createElement("source");
								var sourceNode=audioNode.appendChild(createAudioNode);
								...
								switch(latinLetter){
									case 'A': sourceNode.setAttribute("src","voices/alexa.mp3");break;
									case 'B': sourceNode.setAttribute("src","voices/siriW.mp3");break;
									case 'C': sourceNode.setAttribute("src","voices/cortanaW.mp3");break;
									case 'D': sourceNode.setAttribute("src","voices/siriM.mp3");break;
									case 'E': sourceNode.setAttribute("src","voices/google.mp3");break;
									case 'F': sourceNode.setAttribute("src","voices/robotM.mp3");break;
								}
								
								//sourceNode.setAttribute("src","voices/alexa.wav");
								sourceNode.setAttribute("type","audio/mpeg");
								
								//window.history.pushState("", "", '#'+latinLetter);
							}
						});
						infoBoxOpen = _value;
					});
...
var counter=0;
objKeys=Object.keys(sessionStorage);
var isTrue=false;
...
var randomNum = Math.floor(Math.random() * (max - min + 1)) + min;
	...
	for(d=0;d<Object.values(objKeys).length;d++){
	if(Object.values(objKeys)[d]==randomNum){
		isTrue=true;break;
	}
}
			...
			switch(randomNum){
					...
					case 1: latinLetter='A';break;
					case 2: latinLetter='B';break;
					case 3: latinLetter='C';break;
					case 4: latinLetter='D';break;
					case 5: latinLetter='E';break;
					case 6: latinLetter='F';break;
				}

				window.sessionStorage.setItem(randomNum, latinLetter);
...
//console.log("asdfsdafsdf: "+Object.keys(sessionStorage));
				

				
								
					//console.log("latinSquareLetter "  + latinSquareLetter);
					var task = $(this).find('task' + latinLetter + '[lang="' + selectedLanguage + '"]').html();
					theTask = task;
